"""
Accident Detection from CCTV Footage — Streamlit Dashboard

Tabs:
  Home          — Project overview & dataset info
  Model Results — Metrics table + charts
  Augmentation  — Live interactive augmentation explorer
  Image Predict — Upload image → prediction + confidence
...
try:
    ...
    from keras.applications.resnet_v2 import preprocess_input as resnet_pre
...
.stApp {
  background: linear-gradient(135deg,
    #7B4F00 0%, #C8860A 18%, #F5C518 38%, #FFE87C 55%,
    #F5C518 72%, #C8860A 88%, #7B4F00 100%) fixed;
...
[data-testid="stSidebar"] * { color: var(--gold-light) !important; font-family: 'Raleway', sans-serif !important; }
...
.main .block-container { background: transparent; padding: 2rem 2.5rem; position: relative; z-index: 1; }
...
.metric-box .metric-value { font-family: 'Playfair Display', serif; font-size: 2rem; font-weight: 900; color: var(--charcoal); }
.metric-box .metric-label { font-size: 0.75rem; font-weight: 600; letter-spacing: 1.2px; text-transform: uppercase; color: var(--gold-dark); margin-top: 0.2rem; }

.pred-accident { background: linear-gradient(135deg, #FFEAEA, #FFCCCC); border: 2px solid var(--accent-red); border-radius: 12px; padding: 1.4rem; text-align: center; }
.pred-safe     { background: linear-gradient(135deg, #EAFFF0, #CCFFDD); border: 2px solid var(--accent-green); border-radius: 12px; padding: 1.4rem; text-align: center; }
.pred-label    { font-family: 'Playfair Display', serif; font-size: 1.8rem; font-weight: 900; }
.pred-conf     { font-family: 'Raleway', sans-serif; font-size: 1rem; font-weight: 600; letter-spacing: 1px; margin-top: 0.4rem; }

.conf-bar-wrap         { background: rgba(255,255,255,0.5); border-radius: 50px; height: 14px; width: 100%; margin-top: 0.6rem; overflow: hidden; border: 1px solid rgba(0,0,0,0.08); }
.conf-bar-fill-accident { height: 100%; border-radius: 50px; background: linear-gradient(90deg, #FF6B6B, #C0392B); }
.conf-bar-fill-safe     { height: 100%; border-radius: 50px; background: linear-gradient(90deg, #55EFC4, #1E8449); }

.dataframe { font-family: 'Fira Code', monospace !important; font-size: 0.85rem !important; background: var(--warm-white) !important; }
.dataframe thead th { background: var(--gold-mid) !important; color: var(--charcoal) !important; font-family: 'Raleway', sans-serif !important; font-weight: 700 !important; letter-spacing: 0.8px !important; }
...
.stButton > button:hover { transform: translateY(-2px) !important; box-shadow: 0 6px 20px rgba(180,130,0,0.45) !important; }

.stSlider > div > div > div { background: var(--gold-mid) !important; }
[data-baseweb="select"] { background: var(--cream) !important; border-color: var(--gold-mid) !important; }

.stTabs [data-baseweb="tab-list"] { background: transparent; gap: 0.3rem; }
.stTabs [data-baseweb="tab"] { background: rgba(255,248,200,0.6); border: 1px solid var(--gold-deep); border-radius: 8px 8px 0 0; font-family: 'Raleway', sans-serif; font-weight: 600; color: var(--brown); padding: 0.5rem 1.1rem; }
.stTabs [aria-selected="true"] { background: var(--gold-mid) !important; color: var(--charcoal) !important; border-bottom-color: transparent !important; }

[data-testid="stFileUploader"] { background: rgba(255,248,200,0.7) !important; border: 2px dashed var(--gold-deep) !important; border-radius: 12px !important; }
.stProgress > div > div { background: linear-gradient(90deg, var(--gold-mid), var(--gold-deep)) !important; }
...
[data-testid="stExpander"] { background: rgba(255,248,200,0.6) !important; border: 1px solid var(--gold-deep) !important; border-radius: 10px !important; }
</style>
"""
...
# ── Fallback model builders (for augmented models with broken .keras serialization) ──
...
def build_cnn_bn_dropout_model():
    ...
    for filters in [32, 64, 128]:
        x = keras.layers.Conv2D(filters, (3, 3), padding='same', kernel_initializer=he_init,
...
def _try_load_aug_weights(path: str):
    """Reconstruct aug models from architecture and load weights as a last resort."""
    ...
    model = build_aug_cnn_model() if base == "model_2_augmented_cnn.keras" else build_cnn_bn_dropout_model()
    model(np.zeros((1, 224, 224, 3), dtype=np.float32))
    model.load_weights(weights_path)
    return model


# ── Model loader with multi-strategy Keras 3.3.x bug workarounds ─────────────
...
def load_model_cached(path: str):
    ...
    def _is_flat_tensor_ref(obj):
        return (isinstance(obj, list) and len(obj) == 3
                and isinstance(obj[0], str) and isinstance(obj[1], int) and isinstance(obj[2], int))
    ...
    try:
        ...
        file_bytes["config.json"] = json.dumps(_patch_config(copy.deepcopy(cfg))).encode("utf-8")
        ...
        try:
            return _keras.models.load_model(tmp_keras.name, compile=False, safe_mode=False)
        finally:
            try: os.unlink(tmp_keras.name)
            except Exception: pass
    except Exception:
        pass

    # Strategy 3: patch + strip augmentation Sequential
    try:
        with zipfile.ZipFile(path, "r") as zf_in:
            file_bytes = {n: zf_in.read(n) for n in zf_in.namelist()}

        cfg = _patch_config(json.loads(file_bytes["config.json"].decode("utf-8")))

        def _strip_aug_sequential(config):
            if not isinstance(config, dict) or config.get("class_name") not in ("Functional", "Model"):
                return config
            layers = config.get("config", {}).get("layers", [])
            aug_names = {l["config"]["name"] for l in layers
                         if l.get("class_name") == "Sequential"
                         and any(k in l["config"]["name"].lower() for k in ("aug", "augment"))}
            if not aug_names:
                return config
            kept = [l for l in layers if l["config"]["name"] not in aug_names]
            input_name = next((l["config"]["name"] for l in kept if l.get("class_name") == "InputLayer"), None)
        ...
        file_bytes["config.json"] = json.dumps(_strip_aug_sequential(cfg)).encode("utf-8")
        ...
        with zipfile.ZipFile(tmp2.name, "w", zipfile.ZIP_DEFLATED) as zf_out:
            for name, data in file_bytes.items():
                zf_out.writestr(name, data)
        try:
            return _keras.models.load_model(tmp2.name, compile=False, safe_mode=False)
        finally:
    ...
    # Strategy 4: tf.keras fallback, then weights-only fallback
    try:
...
# ── UI helpers ────────────────────────────────────────────────────────────────
def gold_divider():
    st.markdown("<hr style='border:1.5px solid #D4A017;margin:1rem 0;opacity:0.5'>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:1rem 0'>
      <div style='font-family:"Playfair Display",serif;font-size:1.5rem;font-weight:900;color:#FFE066;letter-spacing:1px'>
        🚨 AccidentAI
      </div>
      <div style='font-size:0.7rem;letter-spacing:2px;text-transform:uppercase;color:#D4A017;margin-top:0.3rem'>
    ...
    model_choice = st.selectbox("Select trained model", list(MODEL_FILES.keys()), index=8)
    ...
    if model_exists:
        ...
        st.warning(f"⚠️ `{model_path}` not found.\nPrediction tabs will show demo mode.")
        model = None
    ...
    <div style='font-size:0.72rem;color:#A07820;line-height:1.6'>
      <b>Classes</b><br>🔴 Accident<br>🟢 Non Accident<br><br>
...
with tabs[1]:
    ...
    st.dataframe(
        df_disp.style
            .set_properties(**{"font-family":"'Fira Code', monospace", "font-size":"0.84rem"})
...
with tabs[2]:
    ...
    Upload any image and interactively control each augmentation transform.
    See exactly how the training pipeline diversifies the dataset to reduce overfitting.
    </div>""", unsafe_allow_html=True)
    ...
    with aug_col1:
        ...
        aug_file = st.file_uploader("Choose an image", type=["jpg","jpeg","png","bmp"], key="aug_upload")
    ...
    with aug_col2:
        ...
        def apply_augmentations(img_arr, seed=None):
            ...
            pil = pil.rotate(rotation * (2*np.random.rand() - 1), resample=Image.BILINEAR, expand=False)
            if zoom != 1.0:
            ...
            pil = ImageEnhance.Brightness(pil).enhance(np.clip(brightness * (0.85 + 0.3*np.random.rand()), 0.1, 3.0))
            pil = ImageEnhance.Contrast(pil).enhance(np.clip(contrast * (0.85 + 0.3*np.random.rand()), 0.1, 3.0))
            if blur_radius > 0:
            ...
            if noise_std > 0:
                arr_out = np.clip(arr_out + np.random.normal(0, noise_std, arr_out.shape), 0, 255)
        ...
        for row_start in range(0, len(all_imgs), 3):
            cols = st.columns(3)
            for j, col in enumerate(cols):
                idx = row_start + j
                if idx < len(all_imgs):
                    with col:
                        st.image(all_imgs[idx], caption=captions[idx], use_column_width=True, clamp=True)

    gold_divider()
    st.markdown('<div class="section-heading">Why Each Augmentation Helps</div>', unsafe_allow_html=True)
    aug_info = [
        ("🪞", "Horizontal Flip",  "Simulates cameras mounted on opposite sides of the road."),
        ("🔄", "Rotation",         "Handles tilted or improperly mounted CCTV cameras."),
        ("🔍", "Zoom",             "Mimics varying camera focal lengths and distances."),
        ("☀️", "Brightness",       "Covers day/night/dusk and different lighting conditions."),
        ("🌫️", "Contrast",         "Handles fog, rain, and overexposed footage."),
        ("📡", "Gaussian Noise",   "Simulates sensor noise and compression artefacts."),
        ("🔲", "Grayscale",        "Supports black-and-white footage from older cameras."),
        ("🔀", "Translation",      "Accounts for accidents at different frame positions."),
    ]
...
with tabs[3]:
    ...
    with img_col1:
        ...
        img_file = st.file_uploader("Supported: JPG · PNG · BMP", type=["jpg","jpeg","png","bmp"], key="img_pred")
        ...
        if img_file:
            ...
            h, w = img_rgb.shape[:2]
            st.markdown(f"<div style='font-size:0.8rem;color:#7A5C20;margin-top:0.5rem'>📐 {w} × {h} px &nbsp;|&nbsp; 🎨 RGB</div>", unsafe_allow_html=True)
        else:
            st.markdown("""
    ...
    with img_col2:
        ...
        if img_file and model:
            ...
            for cls, prob_val, color in [("🔴 Accident", acc_prob, "#C0392B"), ("🟢 Non Accident", nonacc_prob, "#1E8449")]:
                pct = prob_val * 100
                st.markdown(f"""
                <div style='margin-bottom:0.6rem'>
                  <div style='display:flex;justify-content:space-between;font-size:0.88rem;font-weight:600;color:#3D2B00;margin-bottom:0.3rem'>
                    <span>{cls}</span><span>{pct:.1f}%</span>
                  </div>
                  <div style='background:rgba(255,255,255,0.5);border-radius:50px;height:12px;overflow:hidden;border:1px solid rgba(0,0,0,0.08)'>
                    <div style='width:{pct}%;height:100%;border-radius:50px;background:linear-gradient(90deg,{color}88,{color})'></div>
                  </div>
                </div>""", unsafe_allow_html=True)

            if show_raw:
                st.markdown(f"""
                <div style='margin-top:0.8rem;font-family:"Fira Code",monospace;font-size:0.8rem;color:#7A5C20;
                            background:rgba(255,248,200,0.7);padding:0.6rem;border-radius:8px'>
            ...
            st.warning("⚠️ No model loaded. Please place your `.keras` files and restart.")
        else:
            st.markdown("""
            <div style='text-align:center;padding:3rem;color:#A07820'>
              <div style='font-size:3rem'>🤖</div>
              <div style='font-size:0.9rem;margin-top:0.5rem'>Upload an image to see predictions</div>
            </div>""", unsafe_allow_html=True)
    ...
    if batch_files:
        if not model:
            ...
            for i, bf in enumerate(batch_files):
                ...
                rgb    = cv2.cvtColor(cv2.imdecode(fb, cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
...
with tabs[4]:
    ...
    st.markdown("""
    <div class="glass-card">
    Upload a CCTV video clip. Each sampled frame is run through the selected model.
    ...
    with vid_col1:
        vid_file = st.file_uploader("Upload video (MP4 · AVI · MOV · MKV)", type=["mp4","avi","mov","mkv","webm"], key="vid_pred")
        if vid_file:
            st.video(vid_file)
    ...
    with vid_col2:
        if vid_file and run_btn:
            if not model:
                ...
                st.markdown(f"<div style='font-size:0.82rem;color:#7A5C20;margin-bottom:0.8rem;font-family:\"Fira Code\",monospace'>📹 {w_vid}×{h_vid} · {fps_orig:.1f} fps · {total_frames} frames</div>", unsafe_allow_html=True)

                out_path = None
                if annotate_video:
                    out_path = tmp_path.replace(".mp4", "_annotated.mp4")
                    writer   = cv2.VideoWriter(out_path, cv2.VideoWriter_fourcc(*"mp4v"),
                                               max(1.0, fps_orig / sample_rate), (w_vid, h_vid))
                ...
                while True:
                    ...
                    if frame_idx % sample_rate == 0:
                        ...
                        cv2.putText(frame, f"{'ACCIDENT' if is_acc else 'SAFE'}  {conf*100:.0f}%",
                                    (10, 38), cv2.FONT_HERSHEY_DUPLEX, 1.1, (255,255,255), 2, cv2.LINE_AA)
                        cv2.putText(frame, f"Frame {frame_idx}", (10, 56), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1)
                        ...
                        status_text.markdown(f"<span style='font-size:0.8rem;color:#7A5C20'>Analysed {len(frame_results)} frames…</span>", unsafe_allow_html=True)
                ...
                if frame_results:
                    ...
                    with mc1: metric_box("Frames Checked", str(len(frame_results)))
                    with mc2: metric_box("🚨 Accidents",   str(n_acc_v))
                    with mc3: metric_box("✅ Safe",         str(len(frame_results) - n_acc_v))
                    with mc4: metric_box("Avg Confidence", f"{np.mean(confs_all)*100:.0f}%")
                    ...
                    ax.axhline(threshold, color="#D4A017", linestyle="--", linewidth=1.5)
                    ...
                    ax.set_ylim(0, 1.05)
                    ax.legend(handles=[
                        mpatches.Patch(color="#C0392B", label="Accident"),
                        mpatches.Patch(color="#1E8449", label="Safe"),
                    ...
                    if n_acc_v > 0:
                        ...
                        acc_df = pd.DataFrame(
                            [(r[0], r[2]) for r in frame_results if r[1]=="Accident"],
                            columns=["Frame", "Confidence"]
                        )
            ...
            <div style='text-align:center;padding:4rem;color:#A07820;
              ...
              <div style='font-size:1rem;font-weight:600;margin-top:0.8rem'>Upload a CCTV video to begin</div>
              <div style='font-size:0.8rem;margin-top:0.4rem;color:#C8860A'>MP4 · AVI · MOV · MKV · WEBM</div>
            </div>""", unsafe_allow_html=True)
...
gold_divider()
st.markdown("""
<div style='text-align:center;padding:1rem 0 0.5rem;font-family:"Raleway",sans-serif;
            font-size:0.78rem;color:#7A5C20;letter-spacing:0.8px'>
